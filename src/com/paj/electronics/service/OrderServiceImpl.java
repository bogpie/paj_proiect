...
public class OrderServiceImpl implements OrderService {
  ...
  public OrderServiceImpl() {
    ...
    orderPlacedListener = (client, order) -> {
      ...
      Optional<EmailService> emailServ = Optional.ofNullable(emailService);
      emailServ.orElseThrow(() -> new EmailServiceException("Email service is not running")).sendNotificationEmail(Email.builder()
  ...
  public Order createOrder(Client client, ShoppingCart shoppingCart) {
    ...
    shoppingCart.getCartItems().forEach(item -> item.getProduct().setStockUnits(item.getProduct().getStockUnits() - item.getQuantity()));
  ...
  public void updateOrderBoughtProduct(Order order, Product product, Integer quantity) {
    ...
    order.setTotalCost(order.getBoughtProducts().entrySet()
        .stream()
